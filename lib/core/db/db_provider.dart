...
/// =======================================
/// DASHBOARD: Total Stock Value (✅ NEW)
/// =======================================
/// Watches the total stock quantity across all items.
final totalStockQuantityProvider = StreamProvider<int>((ref) {
  final db = ref.watch(dbProvider);
  final sumExpression = db.stockItems.currentStock.sum();
  final query = db.selectOnly(db.stockItems)..addColumns([sumExpression]);
  
  return query.watchSingle().map((row) => row.read(sumExpression) ?? 0);
});

/// =======================================
/// DASHBOARD: Total Outstanding Balance (✅ NEW)
/// =======================================
/// Watches the total outstanding balance (negative = farmers owe us).
final totalOutstandingBalanceProvider = StreamProvider<int>((ref) {
  final db = ref.watch(dbProvider);
  final sumExpression = db.farmers.currentBalance.sum();
  final query = db.selectOnly(db.farmers)..addColumns([sumExpression]);
